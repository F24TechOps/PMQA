      ...
      >
        <thead style={{ backgroundColor: "#f4f5f6" }}>
          <tr>
            <th
              onClick={toggleSort}
              style={{ textAlign: "left", padding: "8px", cursor: "pointer" }}
            >
              Date {sortAsc ? "▲" : "▼"}
            </th>
              ...
              const runDate = run.createdAt
                ? new Date(
                    run.createdAt.seconds * 1000 +
                      run.createdAt.nanoseconds / 1000000
                  )
                : "-";
              return (
                <tr key={run.uploadId + runDate}>
                  ...
                  <td style={{ padding: "8px" }}>
                    {run.cyclrInfo?.accountId || "-"}
                  </td>
                  <td style={{ padding: "8px" }}>
                    {run.cyclrInfo?.cycleId || "-"}
                  </td>
                  <td style={{ padding: "8px" }}>
                    {run.cyclrInfo?.transactionId || "-"}
                  </td>
